...
- (BOOL)scheduleNow
{
    if (![self.timer isValid]) {
        NSLog(@"Could not schedule invalidated MPTimer (%p).", self);
        return NO;
    }
...
- (BOOL)pause
{
    ...
    if (self.isPaused) {
        NSLog(@"No-op: tried to pause an MPTimer (%p) that was already paused.", self);
        return NO;
    }

    if (![self.timer isValid]) {
        NSLog(@"Cannot pause invalidated MPTimer (%p).", self);
        return NO;
    }

    if (![self isScheduled]) {
        NSLog(@"No-op: tried to pause an MPTimer (%p) that was never scheduled.", self);
        return NO;
    }
    ...
    if (secondsLeft <= 0) {
        NSLog(@"An MPTimer was somehow paused after it was supposed to fire.");
    } else {
        NSLog(@"Paused MPTimer (%p) %.1f seconds left before firing.", self, secondsLeft);
    }
...
{
    if (![self.timer isValid]) {
        NSLog(@"Cannot resume invalidated MPTimer (%p).", self);
        return NO;
    }

    if (!self.isPaused) {
        NSLog(@"No-op: tried to resume an MPTimer (%p) that was never paused.", self);
        return NO;
    }

    NSLog(@"Resumed MPTimer (%p), should fire in %.1f seconds.", self.timeInterval);
