...
- (id)initWithURL:(NSURL *)url clickHandler:(MPTelephoneConfirmationControllerClickHandler)clickHandler
{
    if (![url mp_hasTelephoneScheme] && ![url mp_hasTelephonePromptScheme]) {
        ...
        NSLog(@"Processing URL as a telephone URL when %@ doesn't follow the tel:// or telprompt:// schemes", url.absoluteString);
        return nil;
    }
    ...
    if (self = [super init]) {
        ...
        if (!phoneNumber) {
            ...
            if ([phoneNumber length] == 0) {
                NSLog(@"Invalid telelphone URL: %@.", url.absoluteString);
                return nil;
            }
        }
